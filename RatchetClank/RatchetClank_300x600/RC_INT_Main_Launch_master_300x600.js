...
startIntro = function() {
	...
	// RATCHET
	TweenMax.to(intro_ratchet_char, 1, { opacity: 1, ease: Elastic.easeOut.config(1, 0.75), delay:0.3, right:"45px"});
	
	// RATCHET NAME - FLARE
	TweenMax.to(intro_flare, 1, { opacity: 1});
	TweenMax.to(intro_flare, 1, { ease:Power3.easeOut, delay:0.5, left:"50px"});
	TweenMax.to(intro_flare, 1, { opacity: 0, delay: 0.7});
	
	TweenMax.to(intro_ratchet_name, 1, { opacity: 1, ease:Power3.easeOut, delay:0.4});
	
	// (RATCHET MOVES OUT OF WAY)
	TweenMax.to(intro_ratchet_char, 1, { opacity: 1, ease: Elastic.easeOut.config(1, 0.75), delay:1.8, right:"5px"});
	TweenMax.to(intro_ratchet_name, 1, { opacity: 1, ease:Power3.easeOut, delay:1.8, left:"555px"});
	
	// CLANK
	TweenMax.to(intro_clank_char, 1, { opacity: 1, ease: Elastic.easeOut.config(1, 0.75), delay:1.8, left:"6px"});	
	
	// CLANK NAME - FLARE
	TweenMax.to(intro_flare_clank, 1, { scale:2, opacity: 1});
	TweenMax.to(intro_flare_clank, 1, { ease:Power3.easeOut, delay:1.8, left:"50px"});
	...
	TweenMax.to(intro_clank_name, 1, { opacity: 1, ease:Power3.easeOut, delay:3.0, top:"850px"});
	...
	TweenMax.to(intro_flare_tag, 1, { ease:Power3.easeOut, delay:4.2, left:"250px"});
	TweenMax.to(intro_flare_tag, 1, { opacity: 0, delay: 4.4});
	
	// ARE BACK LEAVES
	TweenMax.to(intro_tagline_are, 1, { opacity: 1, ease:Power3.easeOut, left:"650px", delay:4.4});
		
	// SCALE BG
	TweenMax.to(intro_bg, 5.0, {scale:1, delay:0.2});
	
	// NOW START THE RESOLVE
	TweenMax.to(intro, 4.2, {scale:1, delay:0.0, onComplete:function () {
...
startResolve = function() {
	...
	// DATE
	TweenMax.to(resolve_date, 0.5, { opacity:1, ease: Elastic.easeOut.config(1, 0.75), delay:0.6});
	
	// CTA
	TweenMax.to(btn_resolve_cta_hover, 0.5, {  ease: Elastic.easeOut.config(1, 0.75), delay:1, bottom:"13px"});
	TweenMax.to(btn_resolve_cta, 0.5, { opacity: 1, ease:Power3.easeOut, delay:1, bottom:"13px"});
	
	TweenMax.to(pack, 0.5, { opacity: 1, ease:Power3.easeOut, delay:1, bottom:"64px"});
